...
CLUSTER_ID=$(aws emr create-cluster --name autoClusterHive --applications Name=Hadoop Name=Hive --configurations file://conf_emr_hive.json --release-label emr-6.12.0 --use-default-roles \
  ...
  --log-uri s3://bigdatamell/log_hive --ec2-attributes KeyName=emr-keypair,AdditionalMasterSecurityGroups=$MY_SG,SubnetId=$SUBNET_ID --no-auto-terminate --profile usrmll)
echo $CLUSTER_ID
...
hive -f tpcds_query67a.sql >tpcds_query67a.out #concatenar fecha hora y guardar en variable para proximos pasos 
